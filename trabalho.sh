...
extrai() {
    ...
    echo
    cut -f 2 titles.all.tsv | sort | uniq | tee out1.txt # Seleciona a coluna dois e usa pipe para organizar/selecionar os unicos na saida out1.txt
    echo
    echo 02: \"primaryTitle\" e \"originalTitle\" iguais
    awk -F"\t" '{if ($3 == $4) s += 1} END{print s}' titles.all.tsv | tee out2.txt # Compara as colunas 2 e 3, se forem iguais aumenta o contador, cria arquivo out2.txt com o numero de title iguais
    echo
    echo 03: media das avaliacoes entre 1970 e 2000 
    awk -F"\t" '{if (1969 < $6  && $6 < 2001){s += $10; t+= 1}} END{printf("%.4f\n", s/t)}' titles.all.tsv | tee out3.txt # Compara se a coluna 6 está entre os valores (1970 a 2000), recebe em s a nota do filme, e em t a quantidade de filmes, imprime em out3.txt a media das notas
    echo
    echo 04: media das avaliacoes entre 2000 e 2016  
    awk -F"\t" '{if (1999 < $6  && $6 < 2017){s += $10; t+= 1}} END{printf("%.4f\n", s/t)}' titles.all.tsv | tee out4.txt # Compara se a coluna 6 está entre os valores (2000 a 2016), recebe em s a nota do filme, e em t a quantidade de filmes, imprime em out4.txt a media das notas
    echo
    echo 05: generos unicos
    cut -f 9 titles.all.tsv | grep -v "," | grep -v "\\N" | sort | uniq | wc -l | tr -d ' ' | tee out5.txt # Separa a coluna 9 do arquivo, elimina (com grep -v) as linhas que possuem ',' e '\N', organiza (com sort) por ordem alfabetica, elimina (com uniq) as entradas repetidas, conta as linhas com wc -l e imprime o numero de linhas no out5.txt
    echo
    echo 06: titulos classificados como \"Action\"
    cut -f 9 titles.all.tsv | grep "Action" | wc -l | tr -d ' ' | tee out6.txt # Separa a coluna 9 do arquivo, separa todas linhas que possuem a palavra Action, conta as linhas e printa o numero em out6.txt
    echo
